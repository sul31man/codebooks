...
// Fixed parallel kernel with proper shared memory but simpler logic
__global__ void fixed_parallel_simulation_kernel(float* codebook, int rows, int cols, 
                                                int* Ka_values, int num_Ka, int num_sims, 
                                                int* hit_rates) {
    // Simple but effective organization:
    // - Each block processes ONE Ka value
    // - Each thread processes multiple simulations sequentially
    // - Shared memory used for reduction only (safer)
    ...
    // Shared memory for hit count reduction only
    __shared__ int shared_hits[THREADS_PER_BLOCK];
    ...
    // Thread-local arrays (reduced size - only 2 arrays needed)
    ...
    for (int sim = start_sim; sim < end_sim; sim++) {
        ...
        // Generate messages (simplified - no shared memory cooperation)
        for (int user = 0; user < Ka_val; user++) {
            ...
            for (int k = 0; k < N; k++) {
                ...
                for (int j = 0; j < rows; j++) {
                    // Direct global memory access (no shared memory cooperation)
    ...
    if (thread_id == 0) {
        ...
        atomicAdd(&hit_rates[ka_idx], total_hits);
    }
}

// Host function to run fixed parallel simulation
extern "C" void run_parallel_simulation(int* Ka_values, int num_Ka, int num_sims, int* hit_rates) {
    printf("[DEBUG] Starting fixed parallel simulation: num_Ka=%d, num_sims=%d\n", num_Ka, num_sims);
    ...
    // Simple grid configuration: one block per Ka value
    ...
    printf("[DEBUG] Launching fixed kernel: blocks=%d, threads=%d\n", 
           num_blocks, threads_per_block);
    printf("[DEBUG] Shared memory per block: %d bytes (reduction only)\n", 
           (int)(THREADS_PER_BLOCK * sizeof(int) + sizeof(int)));
    
    // Launch fixed kernel
    fixed_parallel_simulation_kernel<<<grid, block, 0, stream>>>(
    ...
    printf("[DEBUG] Fixed parallel simulation completed successfully\n");
