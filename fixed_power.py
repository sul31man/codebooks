...
class SPARC:
    ...
    def inner_coding(self):
        ...
        # Use Gaussian random matrix for A, i.i.d. N(0, 1/n) per paper
        A = np.random.randn(n, L*(2**J)) / np.sqrt(n)
    ...
    def generate_signal(self):
        ...
        self.sparse_vector_true = sparse_vector.copy()  # Store the true sparse vector for error counting
    ...
    def amp_decode(self, T_max: int = 15, tol: float = 1e-6, fixed_threshold: float = 0.5):
        """
       ...
       # print(f"Reconstruction error (L2): {reconstruction_error}")
        ...
        print(f"[AMP] Using fixed threshold: {fixed_threshold}")
        theta = self.sparse_vector.copy()  # or whatever variable holds the AMP output
        detected = np.where(np.abs(theta) > fixed_threshold)[0]
        # Build reconstructed sparse vector
        recon = np.zeros_like(theta)
        recon[detected] = theta[detected]
        # Overwrite self.sparse_vector with reconstruction for error checking
        self.sparse_vector = recon
        # Check structure match
        orig_indices = set(np.where(self.sparse_vector != 0)[0])
        recon_indices = set(detected)
        worked = (orig_indices == recon_indices)
        self.sparse_vector_recon = recon  # Store the reconstructed sparse vector
        return recon, worked      # ŝ  (≈ integers)
    
    def simulate(self, num_sims, fixed_threshold=0.5):
        block_errors = 0
        for trial in range(num_sims):
            self.generate_signal()
            y = self.transmit_signal()
            self.amp_decode(fixed_threshold=fixed_threshold)
            orig_indices = set(np.where(self.sparse_vector_true != 0)[0])
            recon_indices = set(np.where(self.sparse_vector_recon != 0)[0])
            ...
            if trial == 0:
                print("[DEBUG] Original sparse vector (nonzero indices):", sorted(list(orig_indices)))
                print("[DEBUG] AMP reconstructed vector (nonzero indices):", sorted(list(recon_indices)))
                print("[DEBUG] AMP reconstructed vector (raw values):", self.sparse_vector_recon)
                print("[DEBUG] AMP output (pre-threshold):", getattr(self, 'theta', 'theta not stored'))
            if block_error:
                block_errors += 1
            print(f"[DEBUG] Trial {trial}: Missed users: {len(missed)}, False positives: {len(false_positives)}, Block error: {block_error}")
...
def sweep_ebn0_for_ka():
    Ka = 10  # You can change this value as needed
    P_total = 5
    ebn0_range = [1.0 + 0.5*i for i in range(11)]  # Eb/N0 from 1 to 6 dB in 0.5 dB steps
    for Eb_N0_dB in ebn0_range:
        print(f"[Sweep] Ka={Ka}, Eb/N0={Eb_N0_dB:.2f}dB")
        system = SPARC(Ka=Ka, L=32, J=8, B=128, n=19600, P_total=P_total, Eb_N0_dB=Eb_N0_dB)
        success_rate = system.simulate(10)  # 10 trials for speed
        error_rate = 1 - success_rate
        print(f"[Sweep] Ka={Ka}, Eb/N0={Eb_N0_dB:.2f}dB, Block error rate: {error_rate:.3f}")
...
if __name__ == "__main__":
    ...
    Eb_N0_dB = 3.0
    fixed_threshold = 0.5
    print(f"[Single Test] Ka={Ka}, Eb/N0={Eb_N0_dB:.2f}dB, Threshold={fixed_threshold}")
    system = SPARC(Ka=Ka, L=32, J=8, B=128, n=19600, P_total=P_total, Eb_N0_dB=Eb_N0_dB)
    success_rate = system.simulate(10, fixed_threshold=fixed_threshold)
    error_rate = 1 - success_rate
    print(f"[Single Test] Ka={Ka}, Eb/N0={Eb_N0_dB:.2f}dB, Threshold={fixed_threshold}, Block error rate: {error_rate:.3f}")
